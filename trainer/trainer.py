...
from supersuit import color_reduction_v0, frame_stack_v1, resize_v1, reshape_v0
...
class Agent(nn.Module):
    def __init__(self, num_actions):
        ...
        self.network = nn.Sequential(
            self._layer_init(nn.Conv2d(4, 32, 1)),
            nn.MaxPool2d(2),
            nn.ReLU(),
            self._layer_init(nn.Conv2d(32, 64, 1)),
            nn.MaxPool2d(2),
            nn.ReLU(),
            self._layer_init(nn.Conv2d(64, 128, 1)),
            nn.MaxPool2d(2),
            nn.ReLU(),    
            nn.Flatten(),           
            self._layer_init(nn.Linear(1, 512)),
            nn.ReLU()       
        )
    ...
    def get_action_and_value(self, x, action=None):
        ...
        if action is None:
            action = probs.sample(sample_shape=[1,1,1,1])
